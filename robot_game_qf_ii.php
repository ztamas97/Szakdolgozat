	...
	<table>
		<form action="" method="post">
			<tr>
				...
				<td>
					<input type="number" name="tablenum" id="tablenum" min= 2 max=<?php echo $table_num ;?> step=2 required/>
				</td>
			</tr>
			<tr>
				<th>
		...
		for($table=1; $table<=$tablenum; $table++){
			...
			for($act; $act<count($teams); $act++){
				if($num<=$max_per_table){
					$team_id=$teams[$act];
					$sql_sub_i="INSERT INTO `biro-osszesito` (`BOAZ`, `CSID`, `FID`, `KID`, `OSSZPONT`, `ASZTAL`, `SSZAM`, `RIDO`) VALUES (NULL, '$team_id', '$user_tourn', '$qf', '0', '$table', '$num', NULL)";
